...
Grounding the claim instead in per-condition measurements on a scene-disjoint held-out split, full-sequence conditioning leads on trajectory error (ADE 2.43~$\to$~2.32\,m) and on the coupling between generated stopping decisions and the vehicle (0.29~$\to$~0.33), both significant under a paired bootstrap.
...
Our work differs in generating 263D full-body motion in the HumanML3D representation, modelling full-body articulation rather than root position alone, subject to the pose-label limits we quantify in Sec.~\ref{sec:posequality}.

\paragraph{Interaction-aware and retrieval-augmented motion generation.}
ReMoDiffuse~\cite{zhang2023remodiffuse} augments motion diffusion with retrieved nearest-neighbour motions cross-attended into the denoiser, a pattern superficially similar to ours, though our denoiser attends over a concatenated sequence rather than cross-attending, where external sequences condition the denoiser via cross-attention.
...
The improvement is significant: the confidence intervals are non-overlapping (H4 CI: $[3.213, 3.571]$; H2 CI: $[5.503, 5.737]$).
...
Interestingly, R-Precision@1 for H4 (0.548) is \emph{lower} than H2's baseline (0.665).
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    H4 (full-sequence) & 2.320 {\scriptsize $\pm$0.108} & 4.826 {\scriptsize $\pm$0.218} & \textbf{1.291} & \textbf{2.591} \\
    H4, seed B & 2.371 {\scriptsize $\pm$0.116} & 4.916 {\scriptsize $\pm$0.231} & 1.310 & 2.628 \\
    H4, seed C & 2.212 {\scriptsize $\pm$0.107} & 4.571 {\scriptsize $\pm$0.202} & 1.386 & 2.797 \\
    H2 (pooled) & 2.434 {\scriptsize $\pm$0.114} & 4.990 {\scriptsize $\pm$0.225} & 1.482 & 2.979 \\
    H6 (unfrozen) & \textbf{2.227} {\scriptsize $\pm$0.115} & 4.578 {\scriptsize $\pm$0.237} & 1.528 & 3.090 \\
    Regressor (deterministic, $K{=}1$) & 1.800 & 3.671 & --- & --- \\
    \method-IA (+ interaction sampling) & 2.635 {\scriptsize $\pm$0.117} & 5.479 {\scriptsize $\pm$0.238} & 1.707 & 3.505 \\
    \method-IA, epoch-matched to H4 (3399) & 2.578 {\scriptsize $\pm$0.109} & 5.339 {\scriptsize $\pm$0.226} & 1.526 & 3.112 \\
    \method-IA, seed 2 (best-val, epoch 899) & 2.608 {\scriptsize $\pm$0.111} & 5.469 {\scriptsize $\pm$0.239} & 1.732 & 3.533 \\
    \method-IA, seed 2 at epoch 1299 & 2.576 {\scriptsize $\pm$0.115} & 5.355 {\scriptsize $\pm$0.238} & 1.693 & 3.448 \\
    Unconditional (ego zeroed) & 2.880 {\scriptsize $\pm$0.100} & 5.876 {\scriptsize $\pm$0.198} & 1.228 & 2.454 \\
    Unconditional (trained, $p_\mathrm{drop}{=}1$) & 2.990 {\scriptsize $\pm$0.097} & 6.093 {\scriptsize $\pm$0.199} & 1.202 & 2.380 \\
    \quad + interaction sampling (FID 1.43) & 3.005 {\scriptsize $\pm$0.101} & 6.233 {\scriptsize $\pm$0.211} & 1.379 & 2.754 \\
    Vehicle-only text (epoch 3399) & 2.516 {\scriptsize $\pm$0.111} & 5.094 {\scriptsize $\pm$0.224} & 1.375 & 2.753 \\
    Vehicle-only text, FID-best checkpoint (epoch 99) & 2.709 {\scriptsize $\pm$0.095} & 5.527 {\scriptsize $\pm$0.194} & 1.312 & 2.568 \\
    Oracle text (told the pedestrian's behaviour) & 1.899 {\scriptsize $\pm$0.134} & 3.868 {\scriptsize $\pm$0.270} & 1.298 & 2.580 \\
    \bottomrule
  \end{tabular}
\end{table}

Three controls sharpen this reading. A \emph{separately trained} unconditional model, whose FID (3.24) is indistinguishable from H4's, is 29\% worse in ADE and 26\% worse in FDE: identical marginals, no per-condition fidelity --- the trajectory metrics see what FID cannot. An \emph{oracle} text model that is told the pedestrian's own behaviour (Table~\ref{tab:ladder}) reaches ADE 1.90, approaching the $L_2$-optimal regressor while remaining generative; it bounds what a perfect description of the outcome would buy. Finally, \method-IA --- the best model by FID --- is \emph{worse} than H4 on every per-condition metric (ADE 2.64 vs.\ 2.32; minADE$_5$ 1.71 vs.\ 1.29). Interaction-aware sampling improves the marginal but not the conditioning. Its FID-selected checkpoint is trained for a third as long (epoch 1299 vs.\ 3399); an epoch-matched checkpoint recovers part of the gap (ADE 2.58, minADE$_5$ 1.53) but still trails H4 on every metric, so the effect is attributable to the sampling itself rather than to under-training.
...
\paragraph{Significance.} Marginal intervals in Table~\ref{tab:adefde} are percentile bootstraps over the 1{,}190 held-out conditions, and several of them overlap. Because every model is scored on the \emph{same} conditions, the appropriate test is a paired bootstrap on the per-condition difference, which is far more powerful. All comparisons that matter are significant at the 95\% level: full-sequence beats pooled by $0.114$\,m ($[0.028, 0.199]$, $p = 0.011$); both trained priors are worse than H4 by $0.670$\,m and $0.684$\,m ($p < 10^{-4}$), which is the conditioning effect; \method-IA is worse than H4 by $0.315$\,m at its own best checkpoint and $0.288$\,m at its second seed ($p < 10^{-4}$), so its per-condition deficit is not a sampling artifact; and the oracle beats H4 by $0.422$\,m ($p < 10^{-4}$). The one comparison that cuts against our recommendation is H6, which beats H4 by $0.094$\,m ($[0.010, 0.178]$, $p = 0.029$); we return to it below, since H6 buys that accuracy with a $34\%$ collapse in per-condition diversity.
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
    ...
    H4, seed B         & 0.214 & 0.323 & 0.266 & 0.168 \\
    H4, seed C         & 0.171 & 0.318 & 0.192 & 0.162 \\
...
H4's FID varies non-monotonically with guidance and has a clear optimum at CFG~$=10$ (3.842~$\to$~3.392~$\to$~3.547), whereas H2 degrades monotonically as guidance rises (6.603~$\to$~6.963~$\to$~7.400).
...
\method-IA, the $+$pipeline full-sequence cell, has the best marginal; H4 remains the best-conditioned configuration (Sec.~\ref{sec:adefde}).
...
The cross-\emph{architecture} conclusions are unaffected — every H4 seed remains well below the pooled baseline (5.620), and the H2 baseline itself proved training-unstable (two additional H2 seeds were still above FID 10.9 when stopped at roughly 2{,}400 epochs, about half the epoch at which the reported seed peaked, at around four times lower throughput), meaning all our comparisons use the baseline's \emph{best} seed and are conservative in the baseline's favor.
Running the per-condition protocol on all three H4 seeds sharpens the point considerably. While FID ranges over $3.392$--$4.803$ across those seeds, a relative spread of $33\%$, their trajectory error ranges only over $2.212$--$2.371$\,m ($7\%$) and their behavioural separation over $0.318$--$0.333$ ($5\%$). The per-condition instruments are an order of magnitude more stable under reseeding than the metric the field reports, which is a further argument for reading conditioning off them. All three seeds beat the trained unconditional prior decisively (paired bootstrap, $p < 10^{-4}$); against the pooled baseline two of three are significant and the third is not ($p = 0.20$), so we describe full-sequence conditioning as reliably better than no conditioning and, on the evidence of two seeds out of three, better than pooled conditioning.
...
A genuine tradeoff is present too --- full-sequence conditioning generates more diverse motions per condition, which spreads the embedding distribution and lowers retrieval accuracy --- and in AV simulation that diversity is a feature.
...
\begin{itemize}
  ...
  \item \textbf{R-Precision is not cross-model comparable}: H4 scores 0.548 against H2's 0.665, but each model is evaluated in its own encoder's alignment space, so we do not treat this as evidence about conditioning; it is reported for completeness and the evaluator-independent metrics are used instead.
