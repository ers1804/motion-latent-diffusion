...
H6 consistently \emph{underperforms} H4 on the distributional metrics at every evaluated checkpoint: FID worsens by 50\%, R-Precision@1 drops by 36\%, and MultiModality collapses by 34\%.
...
We find that CFG interacts strongly with conditioning granularity: pooled conditioning (H2) shows monotonically increasing FID with CFG scale, while full-sequence conditioning (H4) is has an interior CFG optimum. This suggests that richer conditioning naturally prevents the over-constraint that degrades FID at high CFG.
...
\begin{table}[t]
  ...
  \begin{tabular}{lccccc}
    ...
    Ego trajectory (H4) & continuous, $196\times2$ & 3.39 {\scriptsize $\pm$0.179} & \textbf{2.36} & \textbf{0.33} & 5.79 \\
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    H4 (full-sequence) & 2.360 {\scriptsize $\pm$0.110} & 4.896 {\scriptsize $\pm$0.221} & 1.306 & 2.625 \\
    H4, seed B & 2.375 {\scriptsize $\pm$0.116} & 4.932 {\scriptsize $\pm$0.220} & 1.321 & 2.631 \\
    H4, seed C & 2.258 {\scriptsize $\pm$0.108} & 4.666 {\scriptsize $\pm$0.216} & 1.421 & 2.873 \\
    H2 (pooled) & 2.456 {\scriptsize $\pm$0.114} & 5.032 {\scriptsize $\pm$0.230} & 1.493 & 3.013 \\
    H6 (unfrozen) & 2.262 {\scriptsize $\pm$0.113} & 4.637 {\scriptsize $\pm$0.229} & 1.535 & 3.101 \\
    ...
    \method-IA, epoch-matched to H4 (3399) & 2.615 {\scriptsize $\pm$0.112} & 5.438 {\scriptsize $\pm$0.234} & 1.564 & 3.217 \\
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    H4 (full-sequence) & 0.174 & 0.334 & 0.238 & 0.147 \\
    H4, seed B & 0.216 & 0.325 & 0.264 & 0.167 \\
    H4, seed C & 0.167 & 0.311 & 0.187 & 0.163 \\
    H2 (pooled) & 0.230 & 0.280 & 0.302 & 0.182 \\
    H6 (unfrozen) & 0.217 & 0.351 & 0.182 & 0.178 \\
    \method-IA (+ interaction sampling) & 0.145 & 0.158 & 0.236 & 0.190 \\
    \method-IA, epoch-matched to H4 (3399) & 0.128 & 0.222 & 0.171 & 0.175 \\
...
\begin{table}[t]
  \centering
  \caption{CFG sweep. H4's FID is non-monotonic in guidance with an optimum at CFG 10; H2 degrades monotonically. H2 rows are reported under its original evaluation pipeline (the sweep predates our unified protocol); the within-model \emph{trends}, which are the point of this table, are unaffected.}
  \label{tab:cfg}
  \vspace{4pt}
  \begin{tabular}{lcccc}
    \toprule
...
The $+$pipeline full-sequence cell (\method-IA) has the best marginal of the four; H4 remains the best-conditioned configuration (Sec.~\ref{sec:adefde}).
...
\method-IA, in contrast, \emph{replicates}: a second seed trained with an identical recipe and selected by the same rule reaches FID $2.671 \pm 0.031$ (its best-validation checkpoint is epoch 899) against the reported seed's $2.880 \pm 0.084$, i.e.\ a seed mean of $2.78 \pm 0.15$ --- a spread far tighter than H4's. The per-condition picture replicates as well: the second seed reaches ADE 2.63 and behavioural separation 0.13 (reported seed: 2.66 and 0.16), both short of H4 (2.36 and 0.33), so \method-IA's weaker conditioning fidelity is a property of the interaction-sampling recipe rather than of one unlucky seed. At a matched epoch (1299) the second seed gives FID $3.320 \pm 0.139$, which shows that the two seeds reach their best marginal at different epochs and that per-seed checkpoint selection, not a fixed epoch, is the meaningful protocol.
...
H6 is worse than H4 on FID, R-Precision and MultiModality at \emph{every} evaluated checkpoint (it is, however, slightly better on mean trajectory error; Sec.~\ref{sec:adefde}).
