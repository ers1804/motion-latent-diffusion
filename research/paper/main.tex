...
\begin{table}[h]
  \centering
  \caption{Training-data pipeline 2$\times$2 (FID $\downarrow$, unified eval, 3 replications; best checkpoint per cell). Conditioning granularity dominates; interaction-aware sampling adds a further large gain for the full-sequence model and a marginal one for the pooled baseline. $^\ast$best training-time validation value; definitive replication in progress.}
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    H2 (pooled)        & 5.76$^\ast$ & 5.620 {\scriptsize $\pm$0.117} \\
...
First, the conditioning-granularity gap ($\approx$2.2--2.7 FID) is far larger than the pipeline effect in either row, confirming that full-sequence conditioning — not data curation — is the primary driver.
Second, the pipeline \emph{amplifies} the full-sequence model (3.392~$\to$~2.880, with the best checkpoint arriving at 1/3 the epochs) while barely moving the pooled baseline, suggesting that temporally-resolved conditioning is required to exploit interaction-dense training data.
