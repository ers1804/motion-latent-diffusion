...
We also show that FID alone cannot establish that a condition is \emph{used}: a separately trained unconditional model matches our conditional FID (3.241 vs.\ 3.392), and an unconditional model trained with interaction-aware data sampling reaches 1.43 --- the best marginal we measure --- while remaining ego-blind at chance-level retrieval.
We therefore ground the conditioning claim in per-condition measurements, where full-sequence conditioning also leads: evaluator-independent trajectory error (ADE 2.43~$\to$~2.32\,m) and the coupling between generated stopping decisions and the vehicle (0.29~$\to$~0.33), both on a scene-disjoint held-out split.
...
Interaction-aware data sampling improves the marginal further (FID $2.880 \pm 0.084$, replicated across seeds) but not per-condition fidelity, so we report it as a data-curation result and recommend the frozen-encoder, full-sequence model as the best-\emph{conditioned} system.
Finally, a reference-free audit of all 14{,}172 pseudo-ground-truth sequences --- one canonical skeleton for every pedestrian, and feet that essentially never plant --- bounds what FID can mean on this benchmark and motivates exactly this per-condition protocol.
\end{abstract}
...
\item A negative result with methodological consequences: FID ranks \emph{marginals}, not conditioning. Separately trained unconditional models --- one on the standard recipe (3.241) and one on the interaction-sampling recipe (1.43, the best marginal in the paper) --- match or beat every conditional model while sitting at chance on every conditioning metric. We supply the controls (trajectory ADE/FDE, a behavioral stop/walk probe, and an information ladder over conditioning signals from 0 to 6.09 bits) that do separate the two.
\item A reference-free audit of the pseudo-ground-truth poses on all 14{,}172 sequences, quantifying what the labels do and do not support: the root trajectories are reliable, while body shape is a single canonical template and the feet do not plant. We show the artifact is already present in the motion VAE, and that correcting the labels removes it from reconstruction, which scopes the fix as relabelling rather than an added training loss.
...
\item A design recommendation: frozen ego encoder + full-sequence conditioning (H4) is the best-conditioned architecture for this task, and the one we recommend deploying; interaction-aware sampling is an orthogonal data-side choice that trades per-condition fidelity for marginal realism.
\end{enumerate}
...
\end{equation}
where $\mathbb{1}[\cdot]$ denotes the indicator function, $\phi_t = \operatorname{atan2}(\mathbf{r}_t - \mathbf{g}_t)$ is the pedestrian's bearing relative to the ego vehicle, and $\Delta\phi_t$ is the (wrap-corrected) inter-frame change in bearing.
...
\begin{table}[t]
  ...
  \begin{tabular}{lcc}
    \toprule
    Pipeline stage & Skate ratio & \% frames planted \\
    \midrule
    Pseudo-ground-truth labels & 0.96 & 0.6 \\
    VAE round-trip (encode + decode the labels) & 0.94 & 0.0 \\
    Generated, H4 & 0.94 & 0.0 \\
    Generated, \method-IA & 0.93 & 0.0 \\
    Generated, unconditional + interaction sampling & 0.84 & 0.6 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{lc}
    ...
    Max.\ relative bone-length deviation from that template & $6.3\times10^{-6}$ \\
    Foot-skate ratio, median (0 = footfalls, 1 = sliding) & 0.97 \\
    \quad AVA / nuScenes / Waymo & 0.73 / 0.99 / 0.95 \\
    Sequences never planting a foot in $>$5\% of moving frames & 94.6\% \\
    Sequences with near-static legs (ankle swing $<$ 5\,cm) & 14.1\% \\
    Sequences with ground penetration $>$ 5\,cm & 1.2\% \\
    \midrule
    Variability vs.\ real mocap (ours / HumanML3D std) & Ratio \\
    \midrule
...
\begin{table}[t]
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    None (trained unconditional, $p_\mathrm{drop}{=}1$) & 0 bits & 3.24 {\scriptsize $\pm$0.130} & 2.99 & 0.04 & 5.91 \\
    Vehicle-only text (epoch-matched, 3399) & 5.17 bits & 5.91 {\scriptsize $\pm$0.055} & 2.52 & 0.21 & 5.67 \\
    Oracle text (+ ground-truth pedestrian behaviour) & 6.09 bits & 2.82 {\scriptsize $\pm$0.027} & 1.90 & 0.93 & 6.20 \\
    Ego trajectory (H4) & continuous, $196\times2$ & 3.39 {\scriptsize $\pm$0.179} & \textbf{2.32} & \textbf{0.33} & 5.79 \\
    Ego trajectory + interaction sampling (\method-IA) & continuous, $196\times2$ & \textbf{2.88} {\scriptsize $\pm$0.084} & 2.64 & 0.15 & 5.67 \\
    \bottomrule
  \end{tabular}
  \caption{Information ladder (val split, unified pipeline, 3 replications; CFG 10 for all conditional models, CFG 1 for the unconditional one). FID orders the \emph{marginals}, not the conditioning: a well-trained unconditional model matches the trajectory model, and an oracle that is told the pedestrian's behaviour matches \method-IA. Whether a condition is actually \emph{used} is measured in Secs.~\ref{sec:adefde} and \ref{sec:behavior}. ADE (m) and behavioural separation (held-out split, $K{=}5$) show which signals are actually \emph{used}: the trained prior is ego-blind despite matching H4's FID, the oracle is told the answer, and interaction sampling improves the marginal but not the per-condition fidelity (bold: best non-oracle model). The vehicle-only row is the epoch-matched checkpoint; its FID-best checkpoint (5.12) does not use the condition. The dissociation is sharpest off the ladder: an unconditional model trained with \method-IA's own interaction-sampling recipe reaches FID 1.43, better than every conditional model here, at ADE 3.00 and separation 0.02 (Table~\ref{tab:trivial}).}
  \label{tab:ladder}
\end{table}
...
\begin{table}[h]
  \centering
  \caption{Root-trajectory displacement errors (meters, held-out test split, $K{=}5$ samples per condition, $N{=}1{,}190$). H4 beats the pooled baseline on \emph{all four} metrics (paired bootstrap over conditions: ADE, minADE, and minFDE significant) despite its lower R@1 — direct evidence that the cross-model R@1 gap is an embedding-space artifact. The deterministic regressor minimizes expected L2 by construction yet is unrealistic (FID 35.5, Table~\ref{tab:trivial}); with only 5 samples H4's minADE beats even this L2-optimal predictor. Mean ADE rewards mode concentration (collapsed H6 scores best) while min-of-$K$ rewards coverage — the two must be read together. Bold marks the best ego-trajectory-conditioned model per column; the unconditional and oracle rows are reference bounds (min-of-$K$ rewards the diversity of an unconditional sampler and is not a conditioning test).}
  \label{tab:adefde}
  \vspace{4pt}
  \small
  \begin{tabular}{lcccc}
    ...
    \method-IA (+ interaction sampling) & 2.635 & 5.479 & 1.707 & 3.505 \\
    \method-IA, epoch-matched to H4 (3399) & 2.578 & 5.339 & 1.526 & 3.112 \\
    \method-IA, seed 2 (best-val, epoch 899) & 2.608 & 5.469 & 1.732 & 3.533 \\
    \method-IA, seed 2 at epoch 1299 & 2.576 & 5.355 & 1.693 & 3.448 \\
    Unconditional (ego zeroed) & 2.880 & 5.876 & 1.228 & 2.454 \\
    Unconditional (trained, $p_\mathrm{drop}{=}1$) & 2.990 & 6.093 & 1.202 & 2.380 \\
    \quad + interaction sampling (FID 1.43) & 3.005 & 6.233 & 1.379 & 2.754 \\
    Vehicle-only text (epoch 3399) & 2.516 & 5.094 & 1.375 & 2.753 \\
    Vehicle-only text, FID-best checkpoint (epoch 99) & 2.709 & 5.527 & 1.312 & 2.568 \\
    Oracle text (told the pedestrian's behaviour) & 1.899 & 3.868 & 1.298 & 2.580 \\
    \bottomrule
  \end{tabular}
\end{table}

Three controls sharpen this reading. A \emph{separately trained} unconditional model, whose FID (3.24) is indistinguishable from H4's, is 29\% worse in ADE and 26\% worse in FDE: identical marginals, no per-condition fidelity --- the trajectory metrics see what FID cannot. An \emph{oracle} text model that is told the pedestrian's own behaviour (Sec.~\ref{tab:ladder}) reaches ADE 1.90, approaching the $L_2$-optimal regressor while remaining generative; it bounds what a perfect description of the outcome would buy. Finally, \method-IA --- the best model by FID --- is \emph{worse} than H4 on every per-condition metric (ADE 2.64 vs.\ 2.32; minADE$_5$ 1.71 vs.\ 1.29). Interaction-aware sampling improves the marginal but not the conditioning. Its FID-selected checkpoint is trained for a third as long (epoch 1299 vs.\ 3399); an epoch-matched checkpoint recovers part of the gap (ADE 2.58, minADE$_5$ 1.53) but still trails H4 on every metric, so the effect is attributable to the sampling itself rather than to under-training.
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
    ...
    \method-IA (+ interaction sampling) & 0.142 & 0.152 & 0.238 & 0.193 \\
    \method-IA, epoch-matched to H4 (3399) & 0.129 & 0.228 & 0.168 & 0.176 \\
    \method-IA, seed 2 (best-val, epoch 899) & 0.090 & 0.138 & 0.157 & 0.189 \\
    \method-IA, seed 2 at epoch 1299 & 0.099 & 0.178 & 0.156 & 0.179 \\
    Unconditional      & 0.194 & 0.021 & 0.533 & 0.199 \\
    Unconditional (trained, $p_\mathrm{drop}{=}1$) & 0.232 & 0.041 & 0.590 & 0.199 \\
    \quad + interaction sampling (FID 1.43) & 0.112 & 0.016 & 0.338 & 0.203 \\
    Vehicle-only text (epoch 3399) & 0.230 & 0.214 & 0.387 & 0.183 \\
    Vehicle-only text, FID-best checkpoint (epoch 99) & 0.101 & 0.051 & 0.301 & 0.187 \\
    Oracle text (told the pedestrian's behaviour) & 0.243 & 0.933 & 0.023 & 0.033 \\
    \bottomrule
  \end{tabular}
\end{table}

\method-IA couples behaviour to the ego less than half as strongly as H4 (separation 0.15 vs.\ 0.33) and under-predicts stopping (0.14 vs.\ a base rate of 0.22); at a matched epoch the coupling improves (0.23) while the stopping calibration worsens further (0.13), consistent with the interaction-weighted training distribution shifting behavioural calibration rather than with under-training. Across Tables~\ref{tab:adefde} and \ref{tab:behavior}, H4 --- not the FID-best \method-IA --- is the best-\emph{conditioned} model.
...
\method-IA, in contrast, \emph{replicates}: a second seed trained with an identical recipe and selected by the same rule reaches FID $2.671 \pm 0.031$ (its best-validation checkpoint is epoch 899) against the reported seed's $2.880 \pm 0.084$, i.e.\ a seed mean of $2.78 \pm 0.15$ --- a spread an order of magnitude tighter than H4's. The per-condition picture replicates as well: the second seed reaches ADE 2.61 and behavioural separation 0.14 (reported seed: 2.64 and 0.15), both short of H4 (2.32 and 0.33), so \method-IA's weaker conditioning fidelity is a property of the interaction-sampling recipe rather than of one unlucky seed. At a matched epoch (1299) the second seed gives FID $3.320 \pm 0.139$, which shows that the two seeds reach their best marginal at different epochs and that per-seed checkpoint selection, not a fixed epoch, is the meaningful protocol.
...
\begin{enumerate}
  ...
  Answer: [Yes]. Justification: The headline claims (40\% FID improvement from full-sequence conditioning under a unified evaluation pipeline; further gain from interaction-aware sampling; MultiModality collapse from unfreezing the ego encoder) are supported by replicated evaluations with confidence intervals in Tables~\ref{tab:main}, \ref{tab:pipeline2x2}, and~\ref{tab:h6}, seed-sensitivity reporting for both H4 and \method-IA in Section~\ref{sec:seeds}, and held-out/evaluator-independent verification in Sections~\ref{sec:eval} and~\ref{sec:adefde}.

  \item \textbf{Limitations.} \emph{Does the paper discuss the limitations of the work?}\\
  Answer: [Yes]. Justification: Section~\ref{sec:discussion} lists estimated (not ground-truth) pose labels, restricted conditioning scope, the R@1 gap, lack of physical constraints, evaluator domain shift, and the absence of a perceptual study. Section~\ref{sec:posequality} additionally quantifies the pose-label limitation with a reference-free audit of all 14{,}172 sequences, and Sec.~\ref{sec:seeds} reports seed spread for both models.
