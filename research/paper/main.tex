...
\begin{table}[t]
  ...
  \begin{tabular}{lccc}
    \toprule
    Conditioning signal & Information & FID $\downarrow$ & Diversity \\
    \midrule
    None (trained unconditional, $p_\mathrm{drop}{=}1$) & 0 bits & 3.24 {\scriptsize $\pm$0.130} & 5.91 \\
    Vehicle-only text & 5.17 bits & \emph{in progress} & -- \\
    Oracle text (+ ground-truth pedestrian behaviour) & 6.09 bits & 2.82 {\scriptsize $\pm$0.027} & 6.20 \\
    Ego trajectory (H4) & continuous, $196\times2$ & 3.39 {\scriptsize $\pm$0.179} & 5.79 \\
    Ego trajectory + interaction sampling (\method-IA) & continuous, $196\times2$ & \textbf{2.88} {\scriptsize $\pm$0.084} & 5.67 \\
    \bottomrule
  \end{tabular}
  \caption{Information ladder (val split, unified pipeline, 3 replications; CFG 10 for all conditional models, CFG 1 for the unconditional one). FID orders the \emph{marginals}, not the conditioning: a well-trained unconditional model matches the trajectory model, and an oracle that is told the pedestrian's behaviour matches \method-IA. Whether a condition is actually \emph{used} is measured in Secs.~\ref{sec:adefde} and \ref{sec:behavior}.}
  \label{tab:ladder}
\end{table}
