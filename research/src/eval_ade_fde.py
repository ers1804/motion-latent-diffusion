...
MODELS = {
    ...
    # fair rung: vehicle-only captions (5.17 bits). Best grid cell + epoch-matched cell.
    "text_fair": dict(cfg="configs/config_ego_motion_new_vae_stoch_latent_4_trans_dec.yaml",
                      ckpt=f"{HM}/ego_motion_diffusion_text_egoonly/checkpoints/epoch=99.ckpt",
                      guidance=5,
                      extra=["model.condition=text", "model.denoiser.params.text_encoded_dim=768",
                             ...
                             "DATASET.EGOMOTION.CAPTION_VOCAB=egoonly"]),
    "text_fair_ep3399": dict(cfg="configs/config_ego_motion_new_vae_stoch_latent_4_trans_dec.yaml",
                             ckpt=f"{HM}/ego_motion_diffusion_text_egoonly/checkpoints/epoch=3399.ckpt",
                             guidance=10,
                             extra=["model.condition=text", "model.denoiser.params.text_encoded_dim=768",
                                    "DATASET.EGOMOTION.CAPTIONS_PATH=research/data/synth_captions.json",
                                    "DATASET.EGOMOTION.CAPTION_VOCAB=egoonly"]),
    ...
    # H4 seed variance: do the per-condition conclusions hold across seeds?
    "h4_seedB": dict(cfg="configs/config_ego_motion_new_vae_stoch_latent_4_trans_dec.yaml",
                     ckpt=f"{HM}/ego_motion_diffusion_h4_seedB/checkpoints/epoch=3099.ckpt",
                     guidance=10),
    "h4_seedC": dict(cfg="configs/config_ego_motion_new_vae_stoch_latent_4_trans_dec.yaml",
                     ckpt=f"{HM}/ego_motion_diffusion_h4_seedC/checkpoints/epoch=1799.ckpt",
                     guidance=10),
