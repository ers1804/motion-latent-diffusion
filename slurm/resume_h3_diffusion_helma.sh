#!/bin/bash -l
#SBATCH --job-name=resume_h3_diffusion
#SBATCH --output=/hnvme/workspace/v103fe12-ped_gen/outputs/resume_h3_diffusion_%j.txt
...
# Resume H3 diffusion training (latent-8) from latest checkpoint
# H3 trains ~51s/epoch → 24h wall time → ~1700 epochs per job segment
# Run this after job 329789 completes to continue toward epoch=4399+ (H2's best epoch)
python -m train \
...
--overrides \
    ...
    "TRAIN.RESUME=/hnvme/workspace/v103fe12-ped_gen/models/mld/ego_motion_diffusion_interaction_crop_weighted_latent_8"
