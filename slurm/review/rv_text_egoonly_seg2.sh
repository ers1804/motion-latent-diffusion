#!/bin/bash -l
#SBATCH --job-name=rv_text_egoonly_seg2
#SBATCH --output=/hnvme/workspace/v103fe12-ped_gen/outputs/rv_text_egoonly_seg2_%j.txt
...
--nodebug \
--overrides \
    "NAME=ego_motion_diffusion_text_egoonly" \
    "TRAIN.RESUME=/hnvme/workspace/v103fe12-ped_gen/models/mld/ego_motion_diffusion_text_egoonly" \
    "SEED_VALUE=1234" \
    "model.latent_dim=[4, 256]" \
    "model.condition=text" \
    "model.denoiser.params.text_encoded_dim=768" \
    "model.guidance_uncondp=0.1" \
    "model.guidance_scale=10.0" \
    ...
    "LOGGER.WANDB.RESUME_ID=ego_motion_diffusion_text_egoonly" \
