#!/bin/bash -l
#SBATCH --job-name=rv_uncond_pipeline_seg3
# seg3: pipeline runs pace ~98 ep/h (IA seg1+seg2 ended at ep 4467), so seg2 times out near ep ~4650.
# TIMEOUT never satisfies afterok -> chain the evals on THIS job (afterany:seg2). Resumes from the
# highest epoch=N.ckpt (train.py TRAIN.RESUME) and runs to END_EPOCH=5000 (~350-600 ep, <7 h).
#SBATCH --output=/hnvme/workspace/v103fe12-ped_gen/outputs/rv_uncond_pipeline_seg3_%j.txt
...
--nodebug \
--overrides \
    "NAME=ego_motion_diffusion_uncond_pipeline" \
    "TRAIN.RESUME=/hnvme/workspace/v103fe12-ped_gen/models/mld/ego_motion_diffusion_uncond_pipeline" \
    "SEED_VALUE=1234" \
    "model.latent_dim=[4, 256]" \
    "model.guidance_uncondp=1.0" \
    ...
    "LOGGER.WANDB.RESUME_ID=ego_motion_diffusion_uncond_pipeline" \
